...
curl_response=$(curl -X POST -s -w "%{http_code} %{time_total}\n" -o /dev/null http://localhost:8101/conferences --header 'Content-Type: application/json' --data '{"name": "QCon", "id": "CONF-2022-001"}')
...
echo "" >> ../.results/test.txt
curl_response=$(curl -X POST -s -w "%{http_code} %{time_total}\n" -o /dev/null http://localhost:8101/conferences --header 'Content-Type: application/json' --data '{"name": "Strata", "id": "CONF-2021-002"}')
response_code=$(echo "$curl_response" | awk '{print $1}')
response_time=$(echo "$curl_response" | awk '{print $2}')
if [ "$response_code" != "202" ]; then
    exit 1
...
curl_response=$(curl -X POST -s -w "%{http_code} %{time_total}\n" -o /dev/null http://localhost:8101/conferences --header 'Content-Type: application/json' --data '{"name": "KubeCon", "id": "CONF-2023-003"}')
response_code=$(echo "$curl_response" | awk '{print $1}')
response_time=$(echo "$curl_response" | awk '{print $2}')
if [ "$response_code" != "202" ]; then
    exit 1
...
echo "" >> ../.results/test.txt
curl_response=$(curl -X POST -s -w "%{http_code} %{time_total}\n" -o /dev/null http://localhost:8101/conferences --header 'Content-Type: application/json' --data '{"name": "IBM Think", "id": "CONF-2021-004"}')
response_code=$(echo "$curl_response" | awk '{print $1}')
response_time=$(echo "$curl_response" | awk '{print $2}')
if [ "$response_code" != "202" ]; then
    exit 1
...
echo "" >> ../.results/test.txt
curl_response=$(curl -X POST -s -w "%{http_code} %{time_total}\n" -o /dev/null http://localhost:8101/conferences --header 'Content-Type: application/json' --data '{"name": "Microsoft Ignite", "id": "CONF-2022-005"}')
response_code=$(echo "$curl_response" | awk '{print $1}')
response_time=$(echo "$curl_response" | awk '{print $2}')
if [ "$response_code" != "202" ]; then
    exit 1
...
curl_response=$(curl -X POST -s -w "%{http_code} %{time_total}\n" -o /dev/null http://localhost:8101/conferences --header 'Content-Type: application/json' --data '{"name": "MS Build", "id": "CONF-2023-006"}')
response_code=$(echo "$curl_response" | awk '{print $1}')
response_time=$(echo "$curl_response" | awk '{print $2}')
if [ "$response_code" != "202" ]; then
    exit 1
